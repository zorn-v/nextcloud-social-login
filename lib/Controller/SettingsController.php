...
class SettingsController extends Controller
{
    ...
    public function saveAdmin($new_user_group, $disable_registration, $allow_login_connect, $providers, $openid_providers, $custom_oauth_providers)
    {
        ...
        $this->config->setAppValue($this->appName, 'custom_oauth_providers', json_encode(array_values($custom_oauth_providers)));
    ...
    public function renderPersonal()
    {
        ...
        if (is_array($providers)) {
            foreach ($providers as $provider) {
                $title = $provider['title'];
                $params['providers'][ucfirst($title)] = $this->urlGenerator->linkToRoute($this->appName.'.login.custom_oauth', ['provider'=>$title]);
            }
        }
