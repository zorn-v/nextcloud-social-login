...
<div id="sociallogin" class="section">
    ...
    <h2>
			Custom OAuth2
			<button id="custom_oauth_add" type="button">
				<div class="icon-add"></div>
			</button>
		</h2>
		<div id="custom_oauth_providers">
		<?php foreach ($_['custom_oauth_providers'] as $k=>$provider): ?>
			<div class="provider-settings">
				<div class="custom_oauth-remove">x</div>
				<label>
					<?php p('Title') ?><br>
					<input type="text" name="custom_oauth_providers[<?php p($k) ?>][title]" value="<?php p($provider['title']) ?>" required/>
				</label>
				<br>
        <label>
					<?php p('Authorize url') ?><br>
					<input type="url" name="custom_oauth_providers[<?php p($k) ?>][authorizeUrl]" value="<?php p($provider['authorizeUrl']) ?>" required/>
				</label>
        <br>
        <label>
          <?php p('Token url') ?><br>
          <input type="url" name="custom_oauth_providers[<?php p($k) ?>][tokenUrl]" value="<?php p($provider['tokenUrl']) ?>" required/>
        </label>
        <br>
        <label>
          <?php p('Client Id') ?><br>
          <input type="text" name="custom_oauth_providers[<?php p($k) ?>][clientId]" value="<?php p($provider['clientId']) ?>" required/>
        </label>
        <br>
        <label>
          <?php p('Client Secret') ?><br>
          <input type="text" name="custom_oauth_providers[<?php p($k) ?>][clientSecret]" value="<?php p($provider['clientSecret']) ?>" required/>
        </label>
        <br>
        <label>
          <?php p('Scope') ?><br>
          <input type="text" name="custom_oauth_providers[<?php p($k) ?>][scope]" value="<?php p($provider['scope']) ?>" required/>
        </label>
			</div>
		<?php endforeach ?>
		</div>
  ...
  <div id="custom_oauth_provider_tpl" class="provider-settings" data-new-id="<?php p(count($_['custom_oauth_providers'])) ?>">
		<div class="custom_oauth-remove">x</div>
    <label>
			<?php p('Title') ?><br>
			<input type="text" name="custom_oauth_providers[{{provider_id}}][title]" required/>
		</label>
		<br>
    <label>
			<?php p('Authorize URL') ?><br>
			<input type="url" name="custom_oauth_providers[{{provider_id}}][authorizeUrl]" required/>
		</label>
		<br>
    <label>
			<?php p('Token URL') ?><br>
			<input type="url" name="custom_oauth_providers[{{provider_id}}][tokenUrl]" required/>
		</label>
		<br>
    <label>
			<?php p('Client Id') ?><br>
			<input type="text" name="custom_oauth_providers[{{provider_id}}][clientId]" required/>
		</label>
		<br>
    <label>
			<?php p('Client Secret') ?><br>
			<input type="text" name="custom_oauth_providers[{{provider_id}}][clientSecret]" required/>
		</label>
		<br>
    <label>
			<?php p('Scope') ?><br>
			<input type="text" name="custom_oauth_providers[{{provider_id}}][scope]" required/>
		</label>
		<br>
	</div>
